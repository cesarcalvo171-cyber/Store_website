...
const AVAILABLE_SIZES = ['XS', 'S', 'M', 'L', 'XL', 'Única'];
...
export default function AdminPanel({ 
  ...
  const [mainCategory, setMainCategory] = useState(Object.keys(MENU_STRUCTURE)[0]);
  const [subCategory, setSubCategory] = useState(MENU_STRUCTURE[Object.keys(MENU_STRUCTURE)[0]][0] || '');
  const [detailsInput, setDetailsInput] = useState('');
  const [isFeatured, setIsFeatured] = useState(false);
  
  // Variants Form state
  const [variants, setVariants] = useState([
    { sizes: [], file: null, previewUrl: '' }
  ]);
  ...
  useEffect(() => {
    if (productToEdit) {
      ...
      if (productToEdit.category) {
        const parts = productToEdit.category.split(' - ');
        if (parts.length === 2 && MENU_STRUCTURE[parts[0]]) {
          setMainCategory(parts[0]);
          setSubCategory(parts[1]);
        } else if (MENU_STRUCTURE[productToEdit.category]) {
          setMainCategory(productToEdit.category);
          setSubCategory(MENU_STRUCTURE[productToEdit.category][0] || '');
        } else {
          setMainCategory(Object.keys(MENU_STRUCTURE)[0]);
          setSubCategory(MENU_STRUCTURE[Object.keys(MENU_STRUCTURE)[0]][0] || '');
        }
      } else {
        setMainCategory(Object.keys(MENU_STRUCTURE)[0]);
        setSubCategory(MENU_STRUCTURE[Object.keys(MENU_STRUCTURE)[0]][0] || '');
      ...
      if (productToEdit.product_variants && productToEdit.product_variants.length > 0) {
        setVariants(productToEdit.product_variants.map(v => ({
          sizes: v.sizes || [],
        ...
        setVariants([{ sizes: [], stock_by_size: {}, file: null, previewUrl: '' }]);
      }
    } else {
      // Reset form
      setName('');
  ...
  const handleAddVariant = () => {
    setVariants([...variants, { color_name: '', color_hex: '#000000', sizes: [], stock_by_size: {}, file: null, previewUrl: '' }]);
  };
  ...
  const handleSubmitProduct = async (e) => {
    ...
    if (invalidVariant) {
      alert('Cada imagen debe tener al menos una Talla disponible.');
      return;
    }
    ...
    <div className="fixed inset-0 z-50 bg-[#FAF9F6] dark:bg-zinc-950 flex flex-col overflow-y-auto text-zinc-900 dark:text-zinc-100 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold uppercase tracking-wider">AURA Admin</h1>
        </div>
        ...
        <div className="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8 text-left">
          ...
          <div className="lg:col-span-7 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-6 sm:p-8 shadow-sm">
            <h2 className="text-base font-bold uppercase tracking-widest border-b border-zinc-200 dark:border-zinc-850 pb-3 mb-6">
              {productToEdit ? 'Editar Prenda Existente' : 'Agregar Nueva Prenda'}
            </h2>
            ...
            <form onSubmit={productToEdit ? handleUpdateProduct : handleSubmitProduct} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-1">Nombre de la Prenda</label>
                  <input
                    type="text"
                    required
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder="Ej: Abrigo Trench Camel"
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-1">Categoría Principal</label>
                  <select
                    value={mainCategory}
                    onChange={(e) => {
                      setMainCategory(e.target.value);
                      setSubCategory(MENU_STRUCTURE[e.target.value][0] || '');
                    }}
                    className="w-full text-xs bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-850 px-3 py-2.5 focus:outline-none focus:border-black dark:focus:border-white text-zinc-900 dark:text-white"
                  >
                ...
                {MENU_STRUCTURE[mainCategory] && MENU_STRUCTURE[mainCategory].length > 0 && (
                  <div>
                    ...
                    <select
                      ...
                      className="w-full text-xs bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-850 px-3 py-2.5 focus:outline-none focus:border-black dark:focus:border-white text-zinc-900 dark:text-white"
                    >
                      {MENU_STRUCTURE[mainCategory].map(sub => (
              ...
              <div className="flex items-center gap-2">
                ...
                <label htmlFor="is_featured" className="text-xs font-semibold uppercase tracking-wider text-zinc-600 dark:text-zinc-450 cursor-pointer select-none">
                  Destacar esta prenda en el Hero (Carrusel de Portada)
                </label>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Detalles sobre el ajuste de la prenda, estilo y diseño minimalista..."
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Ej: Material: 100% Lino Orgánico&#10;Corte sastrero contemporáneo&#10;Bolsillos laterales funcionales"
                  className="w-full text-xs bg-zinc-50 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-850 px-3 py-2.5 focus:outline-none focus:border-black dark:focus:border-white text-zinc-900 dark:text-white resize-none"
                />
              </div>
              ...
              <div className="space-y-4 border-t border-zinc-200 dark:border-zinc-850 pt-4">
                <div className="flex justify-between items-center">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Imágenes y Tallas</h3>
                  <button
                    type="button"
                ...
                <div className="space-y-6">
                  {variants.map((variant, vIdx) => (
                    ...
                    >
                      ...
                      {/* Image File Selector — full width now that color fields are removed */}
                      <div>
                        <label className="block text-[9px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-1">Foto de la Prenda</label>
                        <div className="flex items-center gap-2">
                      ...
                      {/* Sizes selection */}
                      <div>
                        <span className="block text-[9px] font-bold uppercase tracking-widest text-zinc-400 dark:text-zinc-500 mb-1.5">Tallas Disponibles para este Color:</span>
              ...
              <div className="space-y-2 pt-4">
                ...
                >
                  {submitLoading ? (
                    <>{productToEdit ? 'Actualizando prenda...' : 'Subiendo producto y variantes...'}</>
                  ) : (
                    <>{productToEdit ? 'Guardar Cambios de la Prenda' : 'Publicar Prenda en la Tienda'}</>
                  )}
                </button>
                    ...
                    <div key={product.id} className="py-3.5 flex gap-3 items-center justify-between">
                      <div className="flex gap-3 min-w-0">
                        ...
                        <div className="min-w-0 flex flex-col justify-center text-left">
                          ...
                          {/* Colors badges preview */}
                          <div className="flex gap-1 mt-1.5">
                            {product.product_variants?.map((v, vIdx) => (
                              <span 
                                key={vIdx} 
