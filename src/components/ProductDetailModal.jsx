...
import { X, Heart, Star, ShoppingBag, Plus, Minus, Check } from 'lucide-react';
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2 text-left">
                ...
                <div className="flex items-baseline gap-3 pt-1">
                  ...
                  {originalPriceVal && (
                    <span className="text-base text-zinc-400 dark:text-zinc-650 line-through font-light">
                      ${originalPriceVal.toFixed(2)}
                    </span>
                  )}
                </div>
                <p className="text-sm text-zinc-500 dark:text-zinc-400 font-light">
                  C${(price * USD_TO_NIO).toFixed(0)} Córdobas (NIO)
                </p>
              </div>
                ...
                {colorsList.length > 0 && (
                  <div className="space-y-2">
                    <span className="text-xs font-bold tracking-widest text-zinc-400 dark:text-zinc-500 uppercase">
                      Color: <span className="text-zinc-900 dark:text-white font-medium ml-1">{colorsList[selectedColorIdx]?.name}</span>
                    </span>
                      ...
                      {colorsList.map((color, idx) => (
                        ...
                        >
                          {selectedColorIdx === idx && (
                            <Check 
                              size={12} 
                              className={color.hex === '#FAF5EF' || color.hex === '#F0ECE3' || color.hex === '#EAE5DF' ? 'text-black' : 'text-white'} 
                            />
                          )}
                        </button>
                      ))}
